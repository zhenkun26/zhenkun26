...
- **[CloudShip](https://github.com/zhenkun26/CloudShip)**：全栈 Vibe Coding 驱动的云原生交付与韧性平台设计——覆盖 IaC（Terraform）、GitOps（Argo CD）、可观测性（Prometheus + Grafana + OTel）、渐进式交付（Canary + 自动回滚）、韧性演练（18 种故障注入场景）与 FinOps，7 阶段实施路线。当前处于设计阶段，作为 AI/Agent 主业之外的云工程能力补充。
  A cloud-native delivery and resilience platform design, built entirely with Vibe Coding — covering IaC (Terraform), GitOps (Argo CD), observability (Prometheus + Grafana + OTel), progressive delivery (Canary + auto-rollback), resilience drills (18 fault injection scenarios), and FinOps, with a 7-phase implementation roadmap. Currently in design phase, complementing the AI/Agent core portfolio with cloud engineering capabilities.
